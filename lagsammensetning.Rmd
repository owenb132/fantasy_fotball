...
Dette er et opimeringsproblem med diverse begresninger. Så hvis jeg kan finne datamateriale over alle spillere, deres fantasipengeverdi, posisjon, lagtilhørighet, popularitet og track record for 2016, bør det gå an å sette sammen noe som er ok - eller bedre enn rein gjetning, i hvert fall.
...
temp = select(filter(df,status=="a"|status=="d"),id,web_name,first_name,second_name,team,element_type,now_cost, selected_by_percent)
...
#plotter mot hverandre
qplot(now_cost_norm, selected_norm, data=temp,facets=~posisjon, geom = c("point", "smooth"), method="lm")
...
### Lagstyrke
Videre kan det også være verdt å se på om spillere på lag som vurderes som sterkere bør få ekstra poeng.
...
temp = group_by(temp,team)%>%
        summarise(verdi_meanteam=mean(verdi))%>%
...
x <- lp ("max", f.obj, f.con, f.dir, f.rhs, all.bin=TRUE)
x
kable(temp[which(x$solution==1),c(3,8,9,10,11,12,25,36)])
...
qplot(now_cost,selected_by_percent,data=temp,color=as.factor(solution))
qplot(now_cost,verdi_teamjustert,data=temp,as.factor(solution))
